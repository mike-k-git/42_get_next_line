...
{
	char	*new_store_fd;
	size_t	new_store_fd_len;

	new_store_fd_len = ft_strlen(new_line + 1);
	new_store_fd = malloc(sizeof(char) * (new_store_fd_len + 1));
	if (!new_store_fd)
	{
		free(store_fd);
		return (NULL);
	}
	copy_n(new_store_fd, new_line + 1, new_store_fd_len);
	new_store_fd[new_store_fd_len] = '\0';
	free(store_fd);
	return (new_store_fd);
}
